...
function Xrp(opt) {
  ...
  const api = new RippleAPI({
    server: opt.test ? 'wss://s.altnet.rippletest.net:51233' : 'wss://s1.ripple.com'
  });
  var connection = null;
  ...
  return {
    ...
    //   const myAddr = params.senderAddress;
    //   const mySecret = params.senderSecretKey;
    ...
    //     },
    //     "destination": {
    //       "address": params.recieverAddress,
    //       "amount": {
    //         "value": amount,
    //         "currency": "XRP"
    //       }
    ...
    //   const VALIDATION_LEDGEROFFSET = 5;
    ...
    //   return new Promise(function(resolve, reject){
    //     prepare().then(() => {
    //       return api.preparePayment(myAddr, myOrder, myInstructions);
    //     }).then(prepared => {
  ...
  function convert(params) {
    return {
      ...
      'mySecret': params.senderSecretKey,
      'myOrder': {
        "source": {
          "address": params.senderAddress,
          "maxAmount": {
